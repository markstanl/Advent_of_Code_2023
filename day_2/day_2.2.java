 ...
 * green, 6 red" it then takes the sum of all of those colors, and makes an array with them. it
 * compares all of the semigames, and makes an integer array with the greatest minimum number for
 * all of the colors, then returns the product of those 3 numbers. It does that for every line and
 * prints that value;
 */
public class D02_5 {
  ...
  /**
   * Returns the integer of the entire game
   * 
   * @param game the line representing the game
   * @return the integer product of the required colors for each game to work
   */
  private static int integerOfGame(String game) {
    ...
    for (String games : gameResets) {
      ...
      games = games.replaceAll("\\s", ""); // removes whitespace

      int[] valOfSemigame = valOfSemigame(games); // if the current max is greater than the overall
      // max, set the max to that value
      if (valOfSemigame[0] > min[0])
        min[0] = valOfSemigame[0];
      if (valOfSemigame[1] > min[1])
        min[1] = valOfSemigame[1];
      if (valOfSemigame[2] > min[2])
        min[2] = valOfSemigame[2];
    }
   ...
   */
  private static int[] valOfSemigame(String semigame) {
